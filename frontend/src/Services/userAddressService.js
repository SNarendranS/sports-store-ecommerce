...
const API_USER_ADDRESS_URL = `${API_BASE_URL}/user/address`

// ✅ Create a new address
const createAddress = async (addressDetail) => {
  try {
    const res = await axios.post(
      `${API_USER_ADDRESS_URL}/create`,
      { addressDetail },
...
const updateAddress = async (addressId, updateDetail) => {
  try {
    const res = await axios.put(
      `${API_USER_ADDRESS_URL}/update`,
      { addressId, updateDetail },
      { headers: getAuthHeaders() }
    )
    return res.data.address // Assuming backend returns `{ address: ... }`
  } catch (error) {
...
const deleteAddress = async (addressId) => {
  try {
    const res = await axios.delete(`${API_USER_ADDRESS_URL}/delete`, {
      headers: getAuthHeaders(),
      data: { addressId }, // axios delete supports sending data this way
    })
    return res.data
  } catch (error) {
